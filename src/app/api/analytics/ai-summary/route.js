  ...
  try {
    ...
    if (!dailyData || dailyData.length === 0) {
      return NextResponse.json({
        success: true,
        analysis: "Not enough data for analysis yet.",
      });
    }

    const busiestDayRate = dailyData.reduce((prev, current) =>
      prev.rateEntries > current.rateEntries ? prev : current,
    );
    const busiestDayTons = dailyData.reduce((prev, current) =>
      prev.totalTons > current.totalTons ? prev : current,
    );
    ...
    const dateWiseReport = dailyData
      .map(
        (d) =>
