...
const ManageDDGSVisibility = dynamic(() =>
  import("../DDGS/ManageDDGSVisibility/ManageDDGSVisibility")
);
const Title = dynamic(() => import("@/components/common/Title/Title"));
const DDGSCompanyPopup = dynamic(() =>
  import("../DDGSCompanyPopup/DDGSCompanyPopup")
);
const SearchBox = dynamic(() =>
  import("@/components/common/SearchBox/SearchBox")
);
const DDGSNotificationsPanel = dynamic(() =>
  import("./DDGSNotificationsPanel/DDGSNotificationsPanel")
);

export default function DDGSRate() {
  const [companies, setCompanies] = useState([]);
  const [loading, setLoading] = useState(false);
  
  const { notifications, refreshNotifications } = useRateNotifications("DDGS");
  ...
  const fetchCompanies = useCallback(async () => {
    try {
      ...
      const res = await axiosInstance.get("/ddgscompany", {
        params: {
  ...
  const openPopup = async (id) => {
    try {
      const res = await axiosInstance.get(`/ddgscompany/${id}`);
  ...
  return (
    <Suspense fallback={<Loading />}>
      <div className="p-4 md:p-8 max-w-7xl mx-auto">
        <Title text="DDGS Commodity Company List" />
        ...
        {showRateList && (
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="w-full lg:w-1/4 h-[80vh] sticky top-4">
              <DDGSNotificationsPanel notifications={notifications} />
            </div>
          </div>
        )}

        {!showRateList && <ManageDDGSVisibility onUpdated={fetchCompanies} />}
