...
export function exportToExcel(rows) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "DDGS Rates");
  XLSX.writeFile(workbook, "ddgs_rates.xlsx");
}
...
function drawHeaderFooter(doc, pageWidth, pageHeight, date, time) {
  ...
  doc.setFontSize(22);
  doc.setTextColor(0);
  doc.text("DDGS RATE REPORT", pageWidth / 2, 85, { align: "center" });
...
export function exportToPDF(rows, columns, selectedDate) {
  ...
  doc.save(`DDGS_Rate_${dateToShow}_${timeToShow}.pdf`);
}
